class World {
  ...
  run() {
    this.loadWorldObjects = setInterval(() => {
  ...
  stopWorld() {
    clearInterval(this.loadWorldObjects);
  }
  ...
  checkCollisions() {
    this.level.chickens.forEach((chicken) => {
      if (
        ...
        !chicken.dead &&
        !this.character.isCollidingTop(chicken)
      ) {
  ...
  checkCollisionsChick() {
    this.level.chicks.forEach((chick) => {
      if (
        ...
        !chick.dead &&
        !this.character.isCollidingTop(chick)
      ) {
        //console.log('Collision with Character', enemy);
        this.character.hit(1);
        this.statusBar.setPercentage(this.character.energy);
  ...
  draw() {
    ...
    this.DrawFixedLayers();
    this.DrawUpperLayers();
